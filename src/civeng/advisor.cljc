(ns civeng.advisor
  "SurveyAdvisor — proposes a field survey or inspection operation
  (record survey data, log inspection findings, flag structural concerns,
  schedule site visits) for a registered project. The advisor is swappable:
  ...
  project provenance or structural-concern risk; `civeng.governor` is the
  independent system that decides whether the proposal may proceed, per
  the itonami actor pattern.

  A proposal is a map:
    {:op :draft-survey-record|:log-inspection-data|:flag-structural-concern|:schedule-site-visit
...
(def ^:private system-prompt
  "You are a civil engineering survey advisor. Given a field survey or
   inspection operation request, propose an :op, an honest :confidence
  ...
  (reify Advisor
    (-advise [_ _store request]
      (let [msgs [{:role :system :content system-prompt}
                  {:role :user :content (str "survey request: " (pr-str request))}]
