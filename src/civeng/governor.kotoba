(ns civeng.governor
  "CivEngGovernor — the independent safety/traceability layer for
  the ISCO-08 3112 civil engineering technician field survey actor. Wired
  as its own `:govern` node in `civeng.actor`'s StateGraph, downstream of
  `:advise` — the Advisor has no notion of project provenance or
  structural-concern risk, so this MUST be a separate system able to
  ...
  README robotics-premise: structural concerns always require human sign-off):
    3. :op :flag-structural-concern.
    4. low confidence (< `confidence-floor`)."
  (:require [civeng.store :as store]))

(def confidence-floor 0.6)
(def ^:private escalating-ops #{:flag-structural-concern})
...
(defn check
  ...
  `store` implementing `civeng.store/Store`. Returns
