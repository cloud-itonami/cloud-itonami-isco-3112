(ns civeng.actor
  "CivEngActor — the ISCO-08 3112 civil engineering technician field
  survey actor as a `langgraph.graph/state-graph` (per ADR-2607011000 /
  CLAUDE.md Actors section). One graph run = one survey operation request
  ...
  The unconditional invariant: the SurveyAdvisor can never directly commit
  a record or dispatch a robot action the CivEngGovernor refuses — every
...
(defn build-graph
  "Build a compiled CivEngActor graph. `store` implements
  `civeng.store/Store`. `advisor` implements `civeng.advisor/Advisor`
...
(defn run-request!
  "Run one survey operation request to completion or interrupt. `thread-id`
